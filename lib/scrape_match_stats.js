 ...
 * @returns 
 */
export default async function scrapeMatchStats(score, statsUrl) {
    return withRetry(async () => {
        try {
            ...
            // Wait for stats section to load (up to 15 seconds)
            await page.waitForSelector('div.gsa_subheader_2', { timeout: 60000 });
            ...
            if (result) {
                ...
                console.error('Score verification failed or no stats found');
                return null;
            }
        } 
        catch (error) {
            console.error(`Error scraping ${statsUrl}: ${error.message}`);
